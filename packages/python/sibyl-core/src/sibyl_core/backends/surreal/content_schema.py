...
CONTENT_SCHEMA_CURRENT_VERSION = 3
...
async def bootstrap_content_schema(client: SurrealContentClient, *, reset: bool = False) -> None:
    ...
    await _assert_content_url_scope_migration_safe(client)
...
async def _assert_content_url_scope_migration_safe(client: SurrealContentClient) -> None:
...
async def _duplicate_count_rows(
    ...
    rows = normalize_records(await client.execute_query(statement))
