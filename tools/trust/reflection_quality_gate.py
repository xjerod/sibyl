...
def _print_receipt(results: Sequence[GateResult], *, echo: Echo) -> None:
    ...
    echo("")
    echo("Reflection Quality Gate Receipt")
...
def run_gate(
    ...
    *,
    ...
    if missing:
        echo("Reflection quality gate is missing required surfaces:")
        for surface in missing:
            echo(f"- {surface}")
        return 2

    active_runner = runner or _real_runner
    echo("Reflection Quality Gate")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run focused reflection-quality checks.")
    parser.add_argument(
        "--list",
        action="store_true",
