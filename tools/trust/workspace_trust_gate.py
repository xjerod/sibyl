...
def _print_receipt(results: Sequence[GateResult], *, echo: Echo) -> None:
    ...
    echo("")
    echo("Workspace Trust Gate Receipt")
...
def run_gate(
    ...
    *,
    ...
    if missing:
        echo("Workspace trust gate is missing required surfaces:")
        for surface in missing:
            echo(f"- {surface}")
        return 2

    active_runner = runner or _real_runner
    echo("Workspace Trust Gate")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run focused workspace trust checks.")
    parser.add_argument(
        "--list",
        action="store_true",
